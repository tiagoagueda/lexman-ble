...
async def async_setup_entry(hass: HomeAssistant, entry: LexmanConfigEntry) -> bool:
    ...
    def _async_update_ble(
        ...
        smart_bulb.set_ble_device_and_advertisement_data(
    ...
    cancel_first_update = smart_bulb.register_callback(lambda *_: startup_event.set())
    coordinator = DataUpdateCoordinator(
        hass,
        _LOGGER,
        config_entry=entry,
        name=smart_bulb.name,
    ...
    try:
        ...
        raise ConfigEntryNotReady(
            ...
            f"Try moving the Bluetooth adapter closer to {smart_bulb.name}"
        ) from ex
    finally:
        cancel_first_update()
